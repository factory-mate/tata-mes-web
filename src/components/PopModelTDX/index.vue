  ...
  >
    ...
    <div class="TDX_main">
      <div class="item_con" v-for="(item, i) in formulaVal" :key="i">
        ...
        <el-select
          v-model="item.termsVal"
          remote
          reserve-keyword
          multiple
          filterable
          placeholder="请选择/搜索"
          :remote-method="DataSearch"
          clearable
          style="width: 160px"
        >
          <el-option
            v-for="item in PageWDData"
            :key="item.cDimensionalityCode"
            :label="item.cDimensionalityName"
            :value="item.cDimensionalityCode"
          />
        </el-select>
        <el-select
          v-model="item.symbolVal"
          placeholder="请选择"
          class="symbolSty"
          clearable
          style="width: 100px"
        ...
        <div>
          <el-select
            v-model="item.contactVal"
            placeholder="请选择"
            clearable
            style="width: 100px; margin-right: 14px"
            v-if="formulaVal.length - i > 1"
            @focus="getData(cLogicSymbol)"
          >
            <el-option
              v-for="item in termsData"
              :key="item.cDictonaryCode"
        ...
        <template
          v-if="formulaVal.length == 1 ? true : formulaVal.length - i > 1"
        >
          <el-button
            type="primary"
            :icon="Plus"
...
const getWDData = async () => {
  WDDdata.value.forEach(
    (item: { cAttributeCode: string; cIncludeModelCode: string }) => {
...
const ArrStrData = () => {
  ...
  formulaVal.value.forEach((item: any) => {
    let itemVal =
      `${item.leftVal == '(' ? item.leftVal : ''}` +
      item.termsVal +
      item.symbolVal +
      item.inputVal +
      `${item.rightVal == ')' ? item.rightVal : ''}` +
      `${item.contactVal != '-' ? item.contactVal : ''}`;
