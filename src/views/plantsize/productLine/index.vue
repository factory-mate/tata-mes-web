<template>
  <!-- 产线档案页面 -->
  <div class="maintain">
    <el-card style="width: 20%; margin-right: 10px" v-if="treeData.length">
      <myTree
        ref="TreeRef"
        :treeData="treeData"
        @treeChangeVal="treeChangeVal"
...
const ClickSearch = (val: any) => {
  ...
  // Conditions.value = filterModel(treeDataForm.value)
...
const treeChangeVal = (data: any) => {
  treeChangeData.value = data.val;
  treeDataForm.value = treeData.value.filter(
    (item: any) => item.Resource.cAttributeTypeCode == 'Filter'
  );
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  treeDataForm.value[0].cAttributeCodeValue = data.val.join('.');
};
</script>
