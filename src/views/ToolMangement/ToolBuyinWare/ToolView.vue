<template>
    ...
    <div class="maintain">
        <el-card >
        <!-- 搜索区域 -->
        <Head-View :Head="head" :row="row" :rowId="rowId" ref="headRef" :disabled="disabled" @RoleBut="RoleBut"></Head-View>
         <!-- 按钮区域 -->
        <div class="bot-btn">
            <ButtonViem :ToolBut="But" @SaveAdd="SaveAdd" @SaveEdit="SaveEdit" @clickEdit="clickEdit"></ButtonViem>
        </div>
        <!-- 表格区域 -->
        <myTable v-if="tabType" :tableData="tableData" :tableColumns="tableColumns" :tableBorder="true"
            :selection="true">
            <template #button>
...
const funTable = (arr: Array<any>) => {
    arr.forEach(item => {
        if (item.Resource.cAttributeTypeCode == 'property'  && item.IsShow==true) {
            let itemData = { checkType: true, label: item.Resource.cAttributeName, prop: item.Resource.cAttributeCode, headerSlot: true, slot: '' }
...
const clickView = (scope: any, obj: any) => {
    console.log(scope,'详情')
    router.push({
        name: 'ReturnQualityXMInfo',
        params:{
            t:Date.now(),
            rowId:scope.row.MID
        },
