        ...
        <el-dialog v-model="dialogFormVisible" :title="title"  draggable :modal="false" :close-on-click-modal="false">
            <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules" label-width="120px" class="demo-ruleForm"
                ...
                <template v-for="item in FormData" :key="item.UID">
                    <el-form-item v-if="item.Resource.cAttributeCode !== 'Data' && item.IsShow== true " :label="item.Resource.cAttributeName + '：'"
                        :prop="item.Resource.cAttributeCode">
                        <el-input v-if="item.cControlTypeCode == 'TextBox'"   v-model="ruleForm[item.cAttributeCode]" :disabled="disabledFun(item)" />
...
watch(props, (newValue, oldValue) => {
    ...
    //输入框禁用
    if (newValue.title == '智能货架档案详情') {
        disabled.value = newValue.disabled
    } else {
        disabled.value = false
    }
    //获取单行数据
    if (newValue.title !== '智能货架档案新增') {
