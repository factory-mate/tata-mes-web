...
const SaveAdd = (obj: any) => {
  obj.pathName = 'LabelUsageConfiguration';
...
const clickEdit = (obj: any) => {
  ...
  // $bus.emit("TabTitleVal", { name: Route.name, title: '标签使用配置编辑' });
  router.push({
    name: 'LabelUsageConfigurationEdit',
    params: {
      t: Date.now(),
      rowId: rowId.value
    },
    state: {
      ...
      row: '',
      pathName: 'LabelUsageConfiguration',
      title: '标签使用配置编辑'
    }
  });
};
