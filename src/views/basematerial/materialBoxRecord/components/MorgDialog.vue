<template>
    <!-- 物料分类新增弹窗  -->
    <div>
        <el-dialog v-model="dialogFormVisible" :title="title"  draggable :modal="false" :close-on-click-modal="false">
            <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules" label-width="120px" class="demo-ruleForm"
                status-icon>
                <template v-for="item in FormData" :key="item.UID">
                    <el-form-item v-if="item.Resource.cAttributeCode !== 'Data' && item.IsShow == true"
                        :label="item.Resource.cAttributeName + '：'" :prop="item.Resource.cAttributeCode">
                        <el-input v-if="item.cControlTypeCode == 'TextBox'" v-model="ruleForm[item.cAttributeCode]"
                            :disabled="disabled" />
                        <el-input v-if="item.cControlTypeCode == 'TextBoxLink'" :disabled="disabled"
                            v-model="ruleForm[item.Resource.cAttributeCode]" placeholder="请输入">
                            <template #append>
                ...
                <el-form-item>
                    <el-button type="primary" @click="submitForm(ruleFormRef, item)" v-for="item in Btn" :key="item.UID">
    ...
    <!-- 搜索弹框 -->
    <searchModel :dialogType="dialogType" :titleName="titleName" :codeType="codeType" :MulitChoose="MulitChoose"
        @ModelClose="ModelClose" @selectData="selectData"></searchModel>
</template>

<script setup lang="ts">
import { ref, toRefs, defineProps, computed,  watch, inject, reactive } from "vue";
import {  ElForm, ElFormItem, ElMessage, ElIcon, ElInput,  FormInstance } from 'element-plus'
...
const { dialogType, titleName, codeType, ButObjData, MulitChoose, AttributeCode } = toRefs(data)
// 搜索弹框事件
...
const headVal = () => {
    ...
    FormData.value.map((item: any) => {
        if(item.Resource.cAttributeCode !== 'Data' && item.IsShow == true){
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
