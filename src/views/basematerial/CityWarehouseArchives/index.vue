...
$bus.on('tableUpData', (v: any) => {
  setTimeout(() => {
    if (v.name == 'CityWarehouseArchives') {
      tableAxios();
    }
  }, 300);
});
...
const clickDelete = (obj: any) => {
  ...
  let idList = [] as any;
  SelectAll.value.forEach((item: { UID: any }) => idList.push(item.UID));
  let data = {
...
const clickView = (scope: any, obj: any) => {
  ...
  window.sessionStorage.setItem(
    'CityWarehouseArchives',
    JSON.stringify(scope.row)
  );
  router.push({
    name: 'CityWarehouseArchivesView',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
    state: {
      ...
      pathName: 'CityWarehouseArchives',
      title: '城市库房档案详情'
    }
  });
};
...
const clickEditTable = (scope: any, obj: any) => {
  ...
  router.push({
    name: 'CityWarehouseArchivesEdit',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
