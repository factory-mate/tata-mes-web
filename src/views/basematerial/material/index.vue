    ...
    <el-card>
      ...
      <div v-show="head.length">
        <Head-View
          ...
          @clickGetDataUser="clickGetDataUser"
          @RoleBut="RoleBut"
        ></Head-View>
      </div>
      <!-- 表格区域 -->
...
const BaseEditModeel = ref('');
//基础信息保存
const SaveAddBase = (obj: any) => {
  // BaseEditModeel.value=obj.cIncludeModelCode
...
const clickGetDataUser = (obj: any) => {
  // window.location.href=window.location.href.replace(/%20/ig,obj.UID)
  // console.log(window.location.href.replace(/%20/ig,obj.UID),'window.location.href');
  // getAddUser(BaseEditModeel.value)
};
//基础信息保存  修改
