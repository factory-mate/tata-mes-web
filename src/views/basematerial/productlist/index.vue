...
const clickAdd = (obj: { cIncludeModelCode: any }) => {
  ...
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  window.sessionStorage.setItem('基本信息', '');
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  window.sessionStorage.setItem('单位信息', []);
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  window.sessionStorage.setItem('库管信息', '');
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  window.sessionStorage.setItem('扩展信息', '');
};
//多选获取UID
  ...
  ExportApi(data).then((res: any) => {
    if (res.status == '200') {
      ...
      const fileName = decodeURI(
        res.headers['content-disposition'] || '物料档案-所有'
      );
...
const ExportOne = async (obj: any) => {
  ...
  ExportApi(data).then((res: any) => {
    if (res.status == '200') {
      const blob = new Blob([res.data], { type: 'application/vnd.ms-excel' });
      const fileName = decodeURI(
        res.headers['content-disposition'] || '物料档案'
      );
      if ('download' in document.createElement('a')) {
        // 非IE下载
        const elink = document.createElement('a');
