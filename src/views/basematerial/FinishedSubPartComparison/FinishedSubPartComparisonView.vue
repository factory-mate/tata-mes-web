        ...
        <!-- <template #button>
                    <el-table-column label="操作" fixed="right" width="160px" align="center">
                        ...
                        <template #default="scope">
                            <el-button v-for='(item,i) in tableButton' :key='i' type="primary" :disabled="disabled" size="small"
                                @click="clickTablBut(scope,item)">删除</el-button>
                        </template>
                    </el-table-column>
                </template> -->
      </myTable>
  ...
  setTimeout(() => {
    let data = {
      ...
      params: {
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
        cProductInvCode: headRef.value.ruleForm.cInvCode
      }
    };
...
const clickTablBut = (scope: any, obj: any) => {
  console.log(obj, '---clickTablBut');

  switch (obj.cAttributeCode) {
    case 'DeleteBody':
      DeleteBody(scope, obj);
      break;
...
const clickEdit = (obj: any) => {
  ...
  // $bus.emit("TabTitleVal", { name: Route.name, title: '成品与子件对照编辑' });
  router.push({
    name: 'FinishedSubPartComparisonEdit',
    params: {
      t: Date.now(),
      rowId: rowId.value
    },
    state: {
      ...
      row: '',
      pathName: 'FinishedSubPartComparison',
      title: '成品与子件对照编辑'
    }
  });
};
