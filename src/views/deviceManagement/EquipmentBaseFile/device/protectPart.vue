<template>
  ...
  <div class="maintain">
    <el-card>
      <!-- 按钮区域 -->
      <div class="bot-btn1">
        <ButtonViem ButtonViem :ToolBut="But" @clickAdd="clickAdd"></ButtonViem>
      </div>
      <Head-View
        :Head="head"
        :row="row"
...
const tableAxios = async () => {
  let data = {
    ...
    data: {
      ...
      Conditions: `cDeviceCode=${row.value.cDeviceCode} && cProgramCode=${row.value.cProgramCode}`
    }
  };
  try {
...
//维护指标
const SetStand = (scope: any, obj: any) => {
  router.push({
    name: 'ProtectTarget',
    params: {
      t: Date.now(),
