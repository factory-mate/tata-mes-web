...
const treeChange = (datas: any, checked: any, indeterminate: any) => {
  ...
  ParentCode.value = data[0].cProjectClassCode;
  if (ParentCode.value) {
    tableAxios();
  }
  // } else {
  ...
  let data = {
    ...
    data: {
      ...
      Conditions: ParentCode.value
        ? `cProjectClassCode=${ParentCode.value}`
        : Conditions.value
        ? Conditions.value
        : ''
    }
...
const clickTableBut = (scope: any, event: any) => {
  switch (event.cAttributeCode) {
    ...
    case 'Stop':
      ...
      break;
    case 'MaintainDocuments':
      MaintenanceFile(scope, event);
      break;
    default:
      break;
  }
...
// 维护文档
const MaintenanceFile = (scope: any, obj: any) => {
  console.log(obj.cIncludeModelCode, '===维护文档----');
