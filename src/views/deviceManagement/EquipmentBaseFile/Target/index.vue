...
const defaultProps = {
  children: 'Child',
  label: 'cDeviceStandardClassName'
};
const treeRef = ref();
...
const treeChange = (datas: any, checked: any, indeterminate: any) => {
  ...
  ParentCode.value = data[0].cDeviceStandardClassCode;
  if (ParentCode.value) {
    tableAxios();
  }
  // } else {
...
const tableAxios = async () => {
  let data = {
    ...
    data: {
      ...
      // "Conditions":ParentCode.value?`cDeviceStandardClassCode=${ParentCode.value}` :'',
      Conditions: ParentCode.value
        ? `cDeviceStandardClassCode=${ParentCode.value}`
        : Conditions.value
        ? Conditions.value
        : ''
    }
