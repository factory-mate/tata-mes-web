...
const treeChange = (datas: any, checked: any, indeterminate: any) => {
  ...
  ParentCode.value = data[0].cProgramClassCode;
  if (ParentCode.value) {
    tableAxios();
  }
  // } else {
...
const tableAxios = async () => {
  ...
  let conditions = [];
  if (ParentCode.value) {
    conditions.push(`cProgramClassCode=${ParentCode.value}`);
  }
  if (Conditions.value) {
...
const clickTableBut = (scope: any, event: any) => {
  ...
  switch (event.cAttributeCode) {
    ...
    case 'Edit':
      ...
      break;
    case 'program_person':
      clickPerson(scope, event);
      break;
    case 'project_file':
      ClickProgram(scope, event);
      break;
    default:
      break;
  }
...
//表格维护人员
const clickPerson = (scope: any, obj: any) => {
  console.log(obj.cIncludeModelCode, 'cIncludeModelCode-----------');
  router.push({
    name: 'ProtectPerson',
    params: {
      t: Date.now(),
      // rowId:scope.row.UID
      rowId: scope.row.cProgramCode
    },
    state: {
...
const ClickProgram = (scope: any, obj: any) => {
  console.log(obj.cIncludeModelCode, 'cIncludeModelCode-----------');
