              ...
              <template #default="scope">
                ...
                >
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item
                        v-for="item in tableButton.filter((v: any,i:number) => i>1)"
                        :key="item.Resource.cAttributeName"
                      >
                        <el-button
                          type="primary"
...
const treeChange = (datas: any, checked: any, indeterminate: any) => {
  ...
  ParentCode.value = data[0].cFileClassCode;
  if (ParentCode.value) {
    tableAxios();
  }
  // } else {
...
const funTable = (arr: Array<any>) => {
  ...
  tableButton.value = tableButton.value.sort(compare('iIndex', true));
};
//表格数据查询
  ...
  let data = {
    ...
    data: {
      ...
      Conditions: ParentCode.value
        ? `cFileClassCode=${ParentCode.value}`
        : Conditions.value
        ? Conditions.value
        : ''
    }
...
// 下载
const clickDownload = (scope: any, obj: any) => {
  console.log(scope, obj);
  const { cFileRoteUrl, cActName, cFileType } = scope.row;
  const linkURL = `${cFileRoteUrl}${cActName}${cFileType}`;
  window.open(linkURL);
};
