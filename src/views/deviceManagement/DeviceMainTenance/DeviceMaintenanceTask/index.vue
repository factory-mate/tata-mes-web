...
$bus.on('tableUpData', (v: any) => {
  setTimeout(() => {
    if (v.name == 'DeviceMaintenanceTask') {
      tableAxios();
    }
  }, 300);
});
...
const clickView = (scope: any, obj: any) => {
  router.push({
    ...
    state: {
      ...
      pathName: 'DeviceMaintenanceTask',
      title: '项目详情'
    }
  });
};
...
const planPersonView = (scope: any, obj: any) => {
  router.push({
    name: 'DeviceMainTenancePersonView',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
...
const ExportAll = async (obj: any) => {
  ...
  ElLoading.service({ lock: true, text: '加载中.....' });
  exportAnalysisHooks(data, '自主保养单-所有');
  ElLoading.service().close();
};
//按钮导出当前页
const ExportOne = async (obj: any) => {
  ...
  ElLoading.service({ lock: true, text: '加载中.....' });
  exportAnalysisHooks(data, '自主保养单');
  ElLoading.service().close();
};
</script>
