...
const total = ref(0);
const orderBy = ref('dPlanDateStart desc,cBatch,cInvCode');
...
const getData = async () => {
  ...
  if (tableQueryConfig.value.Resource && dPlanDateStart.value) {
    getTableData();
  } else {
    loading.close();
  }
...
const resetSearchParams = () => {
  conditions.value = '';
  orderBy.value = 'dPlanDateStart desc,cBatch,cInvCode';
  tableColumns.value = tableSortInit(tableColumns.value);
  queryParams.PageIndex = 1;
  queryParams.PageSize = 20;
  dPlanDateStart.value = null;
  getTableData();
