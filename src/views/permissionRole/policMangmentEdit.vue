  ...
  <div class="maintain">
    <el-card>
      ...
      <Head-View
        ...
        @BtnDAel="BtnDAel"
        @RoleBut="RoleBut"
        @GetTreeRoad="GetTreeRoad"
      ></Head-View>
      <div style="float: right">
        <ButtonViem :ToolBut="Buttwo" @AddForm="AddForm"></ButtonViem>
      </div>
      <myTable
        ref="TABRef"
        :tableData="tableData"
...
const clickView = (val: any) => {
  ...
  $bus.emit('TabTitleVal', { name: Route.name, title: '策略管理详情' });
};
// 获取head数据
const PolicyClass = ref('');
const RoleBut = (v: any) => {
  PolicyClass.value = v.cPolicyTypeCode;
...
const GetTreeRoad = (i: any) => {
  console.log(i, 'i-----------------');
  if (PolicyClass.value !== i) {
    tableData.value = [];
  }
};
...
const TtableAxios = async () => {
  let code = window.sessionStorage.getItem('policyCode');
  let data = {
...
const clickEdit = (obj: any) => {
  ...
  disa.value = false;
  $bus.emit('TabTitleVal', { name: Route.name, title: '策略管理编辑' });
};
</script>
