<template>
    <!-- 新增弹窗  -->
    <div>
        <el-dialog v-model="RdialogFormVisible" :title="title" :close-on-click-modal="false" >
            <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules" label-width="120px" class="demo-ruleForm"
                ...
                <template v-for="item in FormData" :key="item.UID">
                    <el-form-item v-if="item.Resource.cAttributeCode !== 'Data'" :label="item.Resource.cAttributeName + '：'"
                        ...
                        <el-input-number v-if="item.cControlTypeCode == 'TextBox' && item.cDataTypeCode == 'Int'"  v-model="ruleForm[item.Resource.cAttributeCode]"  :placeholder="disabled ? '' : '请输入'"
...
const submitForm = async (formEl: FormInstance | undefined, item: any) => {  
    ...
    await formEl.validate((valid, fields) => {
            if (valid) {
                if(item.cAttributeCode=='DelayOK'){
                    DelayOK(item)
                }
            } else {
...
const DelayOK=(item:any)=>{
    ...
    let data = {
        ...
        data: {
            "DalayCount": ruleForm.value.DalayCount,
            "List_UID":sendId.value,
        }
    }  
