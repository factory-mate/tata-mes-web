  ...
  <div class="maintain">
    <el-card>
      ...
      <myTable
        ...
        :disabled-hide="false"
        show-summary
        :summary-method="() => tableData.length"
      >
      </myTable>
      ...
      <div style="margin-top: 20px">
        ...
        <myTable
          ...
          :tableColumns="tableColumnsExtra"
          :tableBorder="true"
          :selection="false"
          :disabled-hide="false"
          show-summary
          :summary-method="() => tableDataExtra.length"
        >
        </myTable>
      </div>
...
const TABRef = ref();
const TABRefExtra = ref();
//表格数据
const tableData = ref([] as any);
const tableDataExtra = ref([] as any);
//总条数
const total = ref(0);
// 表格配置数据
let dataVal = ref([] as any[]);
const tableColumns = ref(dataVal);
let dataValExtra = ref([] as any[]);
const tableColumnsExtra = ref(dataValExtra.value);
const AxiosData = ref({}) as any;
const AxiosDataExtra = ref({}) as any;
const modelGrid = ref([]) as any;
const modelGridExtra = ref([]) as any;
  ...
  try {
    ...
    if (res.status == 200) {
      res.data.forEach((item: any) => {
        ...
        if (item.cPropertyClassTypeCode == 'Grid2') {
          funTableExtra(
            item[import.meta.env.VITE_APP_key].sort(compare('iIndex', true))
          );
        }
...
const funTableExtra = (arr: Array<any>) => {
  modelGridExtra.value = arr;
  arr.forEach(item => {
    if (item.Resource.cAttributeTypeCode == 'property' && item.IsShow) {
      if (item.IsShow) {
        let itemData = {
        ...
        tableColumnsExtra.value.push(itemData);
        tableColumnsExtra.value.push({
          checkType: true,
          label: '操作',
          slotName: 'button'
        });
        tableColumnsExtra.value = tableColumnsExtra.value.filter(
          (item: { label: any }, index: any, self: any[]) => {
            // 利用findIndex方法找到第一个与当前元素id相等的元素索引
            const i = self.findIndex(
              (t: { label: any }) => t.label === item.label
...
const tableAxiosExtra = async () => {
  if (!rowId.value) {
    return false;
  }
  let data = {
    method: AxiosDataExtra.value.Resource.cHttpTypeCode,
    url:
      AxiosDataExtra.value.Resource.cServerIP +
      AxiosDataExtra.value.Resource.cUrl,
    params: {
      UID: rowId.value
    }
  };
