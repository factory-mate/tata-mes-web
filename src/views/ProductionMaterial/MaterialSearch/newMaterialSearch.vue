                    ...
                    <el-table-column label="操作" fixed="right" width="160px" align="center">
                        ...
                        <template #default="scope">
                            <el-button type="primary" :disabled="disabled" size="small"
                                @click="clickTableHandDel(scope.$index)">删除</el-button>
                        </template>
                    </el-table-column>
                </template> -->
      </myTable>
...
onActivated(() => {
  ...
  if (history.state.title == '领料出库通知详情') {
    disabled.value = true;
  }
});
// 权限按钮
...
const clickEdit = (obj: any) => {
  ...
  $bus.emit('TabTitleVal', { name: Route.name, title: '领料出库通知编辑' });
};
//审核
