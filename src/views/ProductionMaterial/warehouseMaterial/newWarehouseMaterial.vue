...
import { ref, toRefs, watch, reactive, onActivated } from 'vue';
...
onActivated(() => {
  ...
  //   getAddUser(Route.meta.ModelCode);
  // }
  tabVal.value = 'WMS.MaterialApplyForWareHouse.M.View.MaterialList';
  getAddUser(Route.meta.ModelCode);
...
const SaveAdd = (obj: any) => {
  obj.pathName = 'warehouseMaterial';
...
const SaveEdit = (obj: any) => {
  console.log(TABRef.value.tableDataVal, '----TABRef.value.tableDataVal');
  let a = TABRef.value.tableDataVal.some((v: any) => {
    return v.cInvCode == '';
  });
  let b = TABRef.value.tableDataVal.some((v: any) => {
    return v.nQuantity == '';
  });
  let c = TABRef.value.tableDataVal.some((v: any) => {
    return v.cUnitCode == '';
  });
  if (a || b || c) {
    ElMessage({
...
const clickEdit = (obj: any) => {
  ...
  $bus.emit('TabTitleVal', { name: Route.name, title: '仓库领料编辑' });
};
//审核
