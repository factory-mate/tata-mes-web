<template>
  ...
  <div class="maintain">
    ...
    <el-card>
      <!-- 按钮区域 -->
      <ButtonViem :ToolBut="But" @ExportAll="ExportAll" @ExportOne="ExportOne">
      </ButtonViem>
      <!-- 表格区域 -->
      <myTable
        ref="TabRef"
  ...
  let data = {
    ...
    data: {
      ...
      Conditions: Conditions.value
        ? 'cVouchSourceTypeCode = 05 && ' + Conditions.value
        : 'cVouchSourceTypeCode = 05'
    }
  };
  try {
    ElLoading.service({ lock: true, text: '加载中.....' });
...
const ExportAll = async (obj: any) => {
  if (obj.Resource.cUrl) {
    ...
    ElLoading.service({ lock: true, text: '加载中.....' });
    exportAnalysisHooks(data, '销售出库单-所有');
    ElLoading.service().close();
  }
};
//按钮导出当前页
const ExportOne = async (obj: any) => {
  if (obj.Resource.cUrl) {
    ...
    ElLoading.service({ lock: true, text: '加载中.....' });
    exportAnalysisHooks(data, '销售出库单');
    ElLoading.service().close();
  }
};
