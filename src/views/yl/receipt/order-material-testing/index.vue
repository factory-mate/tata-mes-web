...
const handleCalculate = obj => {
  ...
  const data = {};
  router.push({
    name: 'YL-OrderMaterialTestingDetail',
    params: {
      t: Date.now(),
      rowId: selectedIds.value.join(',')
    },
    state: {
      modelCode: obj.cIncludeModelCode,
      pathName: 'YL-OrderMaterialTesting',
      title: '订单测料结果'
    }
  });
};
...
$bus.on('tableUpData', v => {
  if (v.name == 'YL-OrderMaterialTesting') {
    getData();
  }
});
</script>
