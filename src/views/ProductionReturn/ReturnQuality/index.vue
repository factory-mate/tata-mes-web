  ...
  try {
    ...
    if (res.status == 200) {
      ...
      res.data.forEach((item: any) => {
        ...
        if (item.cPropertyClassTypeCode == 'ToolBut' && item.IsShow) {
  ...
  let data = {
    ...
    data: {
      ...
      Conditions: Conditions.value
        ? 'cVouchSourceTypeCode=002 &&' + Conditions.value
...
const clickViewBarcode = (scope: any, obj: any) => {
  router.push({
    ...
    state: {
      ...
      row: JSON.stringify({
        ...scope.row,
        cInvCode: scope.row.Items?.[0].cInvCode,
        MID: scope.row.UID
      }),
      pathName: 'ReturnQuality',
      title: '箱码信息'
