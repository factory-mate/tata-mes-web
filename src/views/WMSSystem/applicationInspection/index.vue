  ...
  <div class="maintain">
    ...
    <el-card>
      ...
      <ButtonViem
        ...
        @ExportAll="ExportAll"
        @ExportOne="ExportOne"
        @PrintCheckLabel="PrintCheckLabel"
      ></ButtonViem>
      <!-- 表格区域 -->
      <myTable
        ref="TabRef"
...
$bus.on('tableUpData', (v: any) => {
  setTimeout(() => {
    if (v.name == 'applicationInspection') {
      tableAxios();
    }
  }, 300);
});
...
const PrintCheckLabel = (obj: any) => {
  ...
  let senid = [] as any;
  selList.value.forEach((item: any) => {
    senid.push(item.UID);
  });
  let data = {
...
const clickEditTable = (scope: any, obj: any) => {
  router.push({
    ...
    state: {
      ...
      pathName: 'applicationInspection',
      title: '检验请单编辑'
    }
  });
};
...
const ExportAll = async (obj: any) => {
  ...
  ElLoading.service({ lock: true, text: '加载中.....' });
  exportAnalysisHooks(ExcelData, '检验申请单-所有');
  ElLoading.service().close();
};
//按钮导出当前页
const ExportOne = async (obj: any) => {
  let ExcelData = {
    ...
    data: {
      ...
      PageSize: queryParams.PageSize
      // "OrderByFileds": OrderByFileds.value,
      // "Conditions": Conditions.value?'cVouchTypeCode = 7 && '+Conditions.value:'cVouchTypeCode = 7'
    }
  };
  ElLoading.service({ lock: true, text: '加载中.....' });
  exportAnalysisHooks(ExcelData, '检验申请单');
  ElLoading.service().close();
};
