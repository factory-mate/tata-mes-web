...
onActivated(() => {
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  if (Route.name === 'IncomingInspectionMaterials') {
  ...
  let data = {
    ...
    data: {
      ...
      Conditions: Conditions.value
        ? Conditions.value + '&& cVouchTypeCode=4'
        : 'cVouchTypeCode=4'
    }
  };
  try {
...
const clickView = (scope: any, obj: any) => {
  router.push({
    ...
    state: {
      ...
      pathName: 'IncomingInspectionMaterials',
      title: '原材料来料检验详情'
    }
  });
};
...
const clickEditTable = (scope: any, obj: any) => {
  router.push({
    name: 'IncomingInspectionMaterialsEdit',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
...
const clickAdd = (obj: { cIncludeModelCode: any }) => {
  router.push({
    name: 'IncomingInspectionMaterialsAdd',
    params: {
      t: Date.now(),
      rowId: ' '
    },
...
const showButton = (item, scope) => {
  console.log(item);
  if (item.cAttributeName === '编辑') {
    if (scope.row.iStatus < 2) {
      return true;
    }
    return false;
  }
