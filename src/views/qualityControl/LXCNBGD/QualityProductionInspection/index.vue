  ...
  let data = {
    ...
    data: {
      ...
      Conditions: Conditions.value
        ? Conditions.value + '&& cVouchTypeCode = 9'
        : 'cVouchTypeCode = 9'
    }
  };
  try {
...
const clickView = (scope: any, obj: any) => {
  router.push({
    name: 'QualityProductionInspectionView',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
    state: {
      ...
      pathName: 'QualityProductionInspection',
      title: '质量产线巡检详情'
    }
  });
};
...
const clickEditTable = (scope: any, obj: any) => {
  router.push({
    name: 'QualityProductionInspectionEdit',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
