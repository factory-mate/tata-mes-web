...
const funTable = (arr: Array<any>) => {
  arr.forEach(item => {
    ...
    if (item.Resource.cAttributeTypeCode == 'method') {
      ...
      console.log(tableButton.value, '--33');
...
// 表格按钮
const clickViewButton = (scope: any, obj: any) => {
  // cVouchTypeCode  4  来料   5 库存
  console.log(scope.row.cVouchTypeCode, '--4');
  let name = '' as any;
  let pathName = '' as any;
  if (scope.row.cVouchTypeCode == '4') {
    name = 'IncomingInspectionMaterialsView';
    pathName = Route.name;
  }
  if (scope.row.cVouchTypeCode == '5') {
    name = 'RawMaterialInventoryInspectionView';
    pathName = Route.name;
  }
  // 来料检验：IncomingInspectionMaterialsView
  // 库存检验：RawMaterialInventoryInspectionView

  // 不合格单：UnqualifiedOrderProcessing
  // 不合格包：UnqualifiedPacketProcessing

  router.push({
    name: name,
    params: {
      t: Date.now(),
      rowId: scope.row.CheckVouchUID
    },
    state: {
