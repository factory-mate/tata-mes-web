...
onActivated(() => {
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  if (Route.name === 'RawMaterialInventoryInspection') {
  ...
  let data = {
    ...
    data: {
      ...
      Conditions: Conditions.value
        ? Conditions.value + '&& cVouchTypeCode = 5'
        : 'cVouchTypeCode = 5'
    }
  };
  try {
...
const clickView = (scope: any, obj: any) => {
  router.push({
    ...
    state: {
      ...
      pathName: 'RawMaterialInventoryInspection',
      title: '原材料库存检验详情'
    }
  });
};
...
const clickEditTable = (scope: any, obj: any) => {
  router.push({
    name: 'RawMaterialInventoryInspectionEdit',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
...
const clickAdd = (obj: { cIncludeModelCode: any }) => {
  router.push({
    name: 'RawMaterialInventoryInspectionAdd',
    params: {
      t: Date.now(),
      rowId: ' '
    },
