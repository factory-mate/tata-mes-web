...
const tableAxios = async () => {
  ...
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  let obj = JSON.parse(window.sessionStorage.getItem('testRating'));
  let data = {
    method: AxiosData.value.Resource.cHttpTypeCode,
    url: AxiosData.value.Resource.cServerIP + AxiosData.value.Resource.cUrl,
    params: {
      cStandardCode: obj.cStandardCode
    }
  };
  try {
    ElLoading.service({ lock: true, text: '加载中.....' });
...
const AddBody = async (obj: any) => {
  ...
  //         res.data.forEach((item:any) => {
  //             if (item.cPropertyClassTypeCode == 'Grid') {
  //                 funTables(item[import.meta.env.VITE_APP_key].sort(compare('iIndex', true)))
  //             }
...
const clickEdit = (obj: any) => {
  ...
  $bus.emit('TabTitleVal', { name: Route.name, title: '检验指标档案编辑' });
};
