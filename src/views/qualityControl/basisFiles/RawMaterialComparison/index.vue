...
onActivated(() => {
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  if (Route.name === 'RawMaterialComparison') {
...
$bus.on('tableUpData', (v: any) => {
  setTimeout(() => {
    if (v.name == 'RawMaterialComparison') {
      tableAxios();
    }
  }, 300);
});
...
const clickEditTable = (scope: any, obj: any) => {
  ...
  window.sessionStorage.setItem(
    'RawMaterialComparison',
    JSON.stringify(scope.row)
  );
  router.push({
    name: 'RawMaterialComparisonEdit',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
    state: {
      modelCode: obj.cIncludeModelCode,
      row: JSON.stringify(scope.row),
      pathName: 'RawMaterialComparison',
      title: '原材料物料对照编辑'
    }
  });
};
