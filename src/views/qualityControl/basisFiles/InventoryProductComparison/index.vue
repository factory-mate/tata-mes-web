...
$bus.on('tableUpData', (v: any) => {
  setTimeout(() => {
    if (v.name == 'InventoryProductComparison') {
      tableAxios();
    }
  }, 300);
});
  ...
  let data = {
    ...
    data: {
      ...
      Conditions: Conditions.value
        ? Conditions.value +
          ' && cConfigTypeCode=00270 && cResourceTypeCode=00273'
        : 'cConfigTypeCode=00270 && cResourceTypeCode=00273'
    }
  };
  try {
...
const clickView = (scope: any, obj: any) => {
  router.push({
    name: 'InventoryProductComparisonView',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
    state: {
      ...
      pathName: 'InventoryProductComparison',
      title: '库存检验产品对照详情'
    }
  });
};
...
const clickEditTable = (scope: any, obj: any) => {
  ...
  window.sessionStorage.setItem(
    'InventoryProductComparison',
    JSON.stringify(scope.row)
  );
  router.push({
    name: 'InventoryProductComparisonEdit',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
...
const clickAdd = (obj: { cIncludeModelCode: any }) => {
  router.push({
    name: 'InventoryProductComparisonAdd',
    params: {
      t: Date.now(),
      rowId: ' '
    },
