...
const SaveAdd = (obj: any) => {
  obj.pathName = 'InspectionTemplate';
...
const clickEdit = (obj: any) => {
  ...
  router.push({
    ...
    state: {
      ...
      pathName: 'InspectionTemplateDetail',
      title: '检验模板编辑'
    }
  });
};
...
const Commit = (obj: any) => {
  ...
  DataApi(data).then(res => {
    if (res.status === 200) {
      ...
      // sendId.value = []
      router.push({ name: 'InspectionTemplate' });
      tagsView.delVisitedView(Route);
      $bus.emit('tableUpData', { name: 'InspectionTemplate' });
    } else {
      console.log('提交失败');
    }
  });
