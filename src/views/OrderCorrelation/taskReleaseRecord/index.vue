...
const handleIssue = (obj: any) => {
  ...
  const names = sendId.value
    .map((item: any) => {
      return tableData.value.find((i: any) => i.UID === item).cPlanReciverName;
    })
    .join();

  ElMessageBox.confirm('确定下发至：' + names + '?', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    ...
    const loading = ElLoading.service({ lock: true, text: '正在下发.....' });
    DataApi(data)
      .then(res => {
        if (res.status === 200) {
          tableAxios();
        ...
        ElMessageBox.alert(res.msg || res.errmsg[0].Value, '下发结果', {
          confirmButtonText: '确定'
        });
      })
      .finally(() => {
...
const data = reactive({
  ...
  dialogTitle: '编辑',
  Conditions: '',
  OrderByFileds: 'dSendDateTime desc'
});
