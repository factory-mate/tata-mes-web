...
$bus.on('tableUpData', (v: any) => {
  setTimeout(() => {
    if (v.name == 'productionChangeOrder') {
      tableAxios();
    }
  }, 300);
});
...
const getData: any = async (val: string) => {
  try {
    ...
    if (res.status == 200) {
      ...
      tableButton.value = [];
      res.data.forEach(
        (item: { cPropertyClassTypeCode: string; Parms: any }) => {
  ...
  let data = {
    ...
    data: {
      ...
      Conditions: Conditions.value
        ? 'cVouchTypeCode = 7 && ' + Conditions.value
        : 'cVouchTypeCode = 7'
    }
  };
  try {
...
const clickView = (scope: any, obj: any) => {
  router.push({
    ...
    state: {
      ...
      pathName: 'productionChangeOrder',
      title: '总部变更单详情'
    }
  });
};

const SelectDataUID = ref([]) as any;
//多选获取UID
const handleSelectionChange = (arr: any) => {
  SelectDataUID.value = arr;
...
const Enter = (obj: any) => {
  let UID = [] as any;
  SelectDataUID.value.forEach((item: { UID: any }) => UID.push(item.UID));
  if (UID.length <= 0) {
    ElMessage({
      type: 'info',
      message: '请勾选数据'
