...
const clickTableBut = (scope: any, event: any) => {
  ...
  switch (event.cAttributeCode) {
    ...
    case 'Scheduling':
      ...
      break;
    case 'CreateTask':
      clickCreateTask(scope, event);
      break;
    case 'Distribute':
      clickDistribute(scope, event);
      break;
    case 'SetLine':
      SetLine(scope, event);
      break;
  ...
  let data = {
    ...
    data: {
      ...
      Conditions: Conditions.value
        ? 'cVouchTypeCode= 01 &&' + Conditions.value
        : 'cVouchTypeCode= 01'
    }
  };
  try {
...
//表格创建任务
const clickCreateTask = (scope: any, obj: any) => {
  console.log(scope, obj, '创建任务---');
};
//表格按钮下发
const clickDistribute = (scope: any, obj: any) => {
  console.log(scope, obj, '下发--');
};
// 表格按钮详情
...
const SetLine = (scope: any, obj: any) => {
  const senid = scope.row.UID;
  let data = {
    method: obj.Resource.cHttpTypeCode,
    url: obj.Resource.cServerIP + obj.Resource.cUrl,
