<template>
  ...
  <div class="maintain">
    ...
    ></Odialogs>
    <ExportDialog ref="ExportDiaRef" :objArr="ExportDatas"></ExportDialog>
...
$bus.on('tableUpData', (v: any) => {
  setTimeout(() => {
    if (v.name == 'FactoryCancellationNote') {
      tableAxios();
    }
  }, 300);
});
...
const tableAxios = async () => {
  loading.value = true;
  let valDat: any = '';
  // if(Route.name=='FactoryCancellationNote'){
  ...
  let data = {
    ...
    data: {
      ...
      Conditions: Conditions.value
        ? 'cVouchTypeCode = 9 && ' + Conditions.value
        : 'cVouchTypeCode = 9'
    }
  };
  try {
...
const clickEditTable = (scope: any, obj: any) => {
  router.push({
    name: 'FactoryCancellationNoteEdit',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
...
const clickAdd = (obj: { cIncludeModelCode: any }) => {
  router.push({
    name: 'newFactoryCancellationNote',
    params: {
      t: Date.now(),
      rowId: ' '
    },
