...
onActivated(() => {
  ...
  if (history.state.row) {
    ...
    if (row.value !== JSON.parse(history.state.row) && !calledAPI) {
  ...
  if (ToolData.length) {
    ...
    DataApi(dataVal).then((res: any) => {
      if (res.data.cPropertyClassTypeCode == 'ToolBut') {
        if (res.data.cModelCode == 'MES.PRODUCTVOUCH.M.View') {
          FirBut.value = res.data[import.meta.env.VITE_APP_key].sort(
            compare('iIndex', true)
          );
        } else {
  ...
  arr.forEach(item => {
    if (item.Resource.cAttributeTypeCode == 'property' && item.IsShow) {
      let itemData = {
        ...
        DefinedParm3: item.DefinedParm3
      };
...
const ChangeOrde = (obj: any) => {
  ...
  router.push({
    ...
    params: {
      ...
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      rowId: headRef.value.ruleForm.UID
    },
    state: {
      ...
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      row: JSON.stringify(headRef.value.ruleForm),
...
const BatchInspection = (obj: any) => {
  let dataVal = {
    ...
    data: {
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      iDs: [headRef.value.ruleForm.UID]
    }
  };
  ElMessageBox.confirm('确定审单?', '提示', {
    confirmButtonText: '确定',
