<template>
    <!-- 新增弹窗  -->
    <div>
        <el-dialog v-model="RdialogFormVisible" :title="title" :close-on-click-modal="false"  draggable :modal="false">
            <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules" label-width="120px" class="demo-ruleForm"
                ...
                <template v-for="item in FormData" :key="item.UID">
                    <el-form-item v-if="item.Resource.cAttributeCode !== 'Data'" :label="item.Resource.cAttributeName + '：'"
                        ...
                        <el-input
                            v-if="item.cControlTypeCode == 'RichText'"
                            :rows="2" type="textarea" v-model="ruleForm[item.Resource.cAttributeCode]"
                            :placeholder="disabled ? '' : '请输入'"  clearable style="width: 100%;" />
