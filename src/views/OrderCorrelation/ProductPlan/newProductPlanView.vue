...
const funTable = (arr: Array<any>) => {
  modelGrid.value = arr.map(i => {
    if (i.Resource.cAttributeCode === 'cLamda') {
      return { ...i, cControlTypeCode: 'RichText', cDataTypeCode: 'Nvarchar' };
    }
    return i;
  });
  arr.forEach(item => {
...
const PickOrder = (obj: any) => {
  // console.log(headRef.value.ruleForm.UID,"----ruleFormruleForm");
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  const senid = headRef.value.ruleForm.UID;
  let data = {
