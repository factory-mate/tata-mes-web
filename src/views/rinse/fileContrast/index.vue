<template>
  ...
  <div class="maintain">
    ...
    <el-card>
      <!-- 打印demo -->
      <!-- <el-button @click="stausclick" v-print="'#printMe'" type="primary" plain>打印111</el-button>
            <iframe ref="frame" frameborder="0" scrolling="no" style="margin: 0px;padding: 0px;width: 0px;height: 0px;"></iframe>
            <div ref="printDiv" style="display: none;">
                <div v-for="item in demo" :key="item.val" style="page-break-after:always;border: 1px solid red;margin-top: 20px;" >
                    <p>名称：{{ item.name }}</p>
                    <p>数据：{{ item.val }}</p>
                </div>
            </div> -->
      <!-- TSC TTP-224 PRO 打码最大安全范围320*210 -->
      <!-- <el-button @click="stausclick" v-print="'#printMe'" type="primary" plain>打印222</el-button>
            <div id="printMe" style="background:#eee;width:320px;height:210px;border:1px solid #000">
                <p>123456789123456789123456789123456789123</p>
                <p>2</p>
                <p>33</p>
                <p>444</p>
                <p>5555</p>
...
$bus.on('tableUpData', (v: any) => {
  setTimeout(() => {
    if (v.name == 'fileContrast') {
      tableAxios();
    }
  }, 300);
});
const stausclick = (v: any) => {
  console.log(v, '-打印Demo');
