  ...
  arr.forEach(item => {
    if (item.Resource.cAttributeTypeCode == 'property' && item.IsShow) {
      let itemData = {
        ...
        DefinedParm4: item.DefinedParm4
      };
...
const tableAxios = async () => {
  ...
  try {
    ...
    if (res.status == 200) {
      tableData.value = res.data.map(i => ({ ...i, nQuantity2: i.nQuantity }));
      ElLoading.service().close();
    } else {
      console.log('请求出错');
      ElLoading.service().close();
...
const clickTableBut = (scope: any, event: any) => {
  ...
  switch (event.cAttributeCode) {
    case 'Split':
      clickSplit(scope, event);
      break;
    case 'Close':
      clickClose(scope, event);
      break;
    case 'DeleteItem':
      clickDeleteItem(scope, event);
      break;
    default:
      break;
  }
...
// table 按钮删除
const clickDeleteItem = (scope: any, obj: any) => {
  //调用子组件的 DelBtn方法
...
//关闭
const clickClose = (scope: any, obj: any) => {
...
//拆分
const clickSplit = (scope: any, obj: any) => {
  if (scope.row.iDefindParm13 == 1) {
    //深拷贝对象，不改变原数据
    let newObj = Object.assign({}, scope.row);
    newObj.iDefindParm13 = '';
    newObj.cVendorName = '';
    newObj.cVendorCode = '';
...
const SaveAdd = (obj: any) => {
  obj.pathName = 'KnifePurchaseAudit';
...
const clickEdit = (obj: any) => {
  ...
  $bus.emit('TabTitleVal', { name: Route.name, title: '采购审核编辑' });
};
...
const Audit = (obj: any) => {
  ...
  DataApi(data).then(res => {
    if (res.status === 200) {
      ...
      router.push({
        name: 'KnifePurchaseAudit'
      });
      $bus.emit('tableUpData', { name: 'KnifePurchaseAudit' });
    } else {
