...
const SaveAdd = (obj: any) => {
  obj.pathName = 'KnifePurchaseRequest';
...
const clickEdit = (obj: any) => {
  ...
  router.push({
    name: 'KnifeAddPurchaseRequestEdit',
    params: {
      t: Date.now(),
      rowId: row.value.UID
    },
    state: {
      ...
      pathName: 'KnifeAddPurchaseRequestView',
      title: '刀具采购申请编辑'
    }
  });
};
...
const Commit = (obj: any) => {
  ...
  DataApi(data).then(res => {
    if (res.status === 200) {
      ...
      // sendId.value = []
      router.push({ name: 'KnifePurchaseRequest' });
      tagsView.delVisitedView(Route);
      $bus.emit('tableUpData', { name: 'KnifePurchaseRequest' });
    } else {
      console.log('提交失败');
    }
  });
