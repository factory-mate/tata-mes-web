...
$bus.on('tableUpData', (v: any) => {
  setTimeout(() => {
    if (v.name == 'KnifePurchaseReturnStock') {
      tableAxios();
    }
  }, 300);
});
  ...
  let data = {
    ...
    data: {
      ...
      Conditions: Conditions.value
        ? 'UID=' + rowId.value + '&&' + Conditions.value
        : 'UID=' + rowId.value
    },
    params: {
      UID: rowId.value
...
const clickView = (scope: any, obj: any) => {
  window.sessionStorage.setItem('caigoutuihuo', JSON.stringify(scope.row));
  router.push({
    name: 'KnifePurchaseReturnStockDetailInfo',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
    state: {
      ...
      pathName: 'KnifePurchaseReturnStock',
      title: '箱码信息'
    }
  });
};
