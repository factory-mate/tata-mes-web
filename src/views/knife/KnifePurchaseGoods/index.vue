...
onActivated(() => {
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  if (Route.name === 'KnifePurchaseGoods') {
...
const clickView = (scope: any, obj: any) => {
  router.push({
    name: 'KnifeNewPurchasedGoodsView',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
...
const clickEditTable = (scope: any, obj: any) => {
  ...
  router.push({
    name: 'KnifeNewPurchasedGoodsEdit',
    params: {
      t: Date.now(),
      rowId: scope.row.UID
    },
...
const CustomExport = (obj: any) => {
  ...
  ElLoading.service({ lock: true, text: '加载中.....' });
  exportAnalysisHooks(data, '刀具到货单');
  ElLoading.service().close();
};
...
const ExportAll = async (obj: any) => {
  ...
  ElLoading.service({ lock: true, text: '加载中.....' });
  exportAnalysisHooks(data, '刀具到货单-所有');
  ElLoading.service().close();
};
//按钮导出当前页
